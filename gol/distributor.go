...
func distributor(p Params, c distributorChannels) {
	...
	for i := 0; i < p.ImageHeight; i++ {
		for k := 0; k < p.ImageWidth; k++ {
			world[i][k] = <-c.ioInput //world recieve the image from io
		}
	}
	for i := 0; i < p.ImageHeight; i++ {
		for k := 0; k < p.ImageWidth; k++ {
	...
	turn := p.Turns
	if turn != 0 {
		newWorld := make([][]uint8, p.ImageHeight) //new world is to store next state
		for i := range newWorld {
			newWorld[i] = make([]uint8, p.ImageWidth)
		}
		ticker := time.NewTicker(2 * time.Second)
		liveCell := len(calculateAliveCells(world))
		completed := 0 //used to store completed turn, since t is not exactly the turn completed
			...
			for {
				...
				default:
					if p.Threads == 1 {
						...
						for i := 0; i < p.Threads; i++ {
							part := <-parts[i] //part ass sync tool
							if i != p.Threads-1 {
...
func saveWorldState(c distributorChannels, p Params, world [][]uint8, fileName string, completed int) {
	c.ioCommand <- ioCommand(0)
	c.ioFilename <- fmt.Sprintf("%s-%d", fileName, completed)
	for i := 0; i < p.ImageHeight; i++ {
		for k := 0; k < p.ImageWidth; k++ {
			c.ioOutput <- world[i][k]
		}
